"""
pipeline.py — Greedy-matching pipeline (shared between local and cloud).
"""
...
def greedy_match(df_count_house: pd.DataFrame, df_select_house: pd.DataFrame,
    ...
    """
    Match select eggs to count events for one house.
    Assigns Bin (belt position) and Line (belt line number) to each select egg.
    """
    ...
    for _, row in dc.iterrows():
        ...
        if "Line" in row.index:
            dx.loc[row["Xi"]:row["Xe"], "Line"] = row["Line"]
